...
input_test = """be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb |
...
def first_star():
    ...
    for x in input_list:
        y = x.split("|")[1].split(' ')
        res += len([num for num in y if len(num) != 5 and len(num) != 6 and len(num) != 0])
...
def second_star():
    ...
    for x in input_list:
        ...
        for dig in y:
            if len(dig) == 6:
                if diff_num(digits[1], dig):
                    digits[6] = sorted(dig)
                elif diff_num(digits[4], dig):
                    digits[0] = sorted(dig)
                else:
                    digits[9] = sorted(dig)
            elif len(dig) == 5:
                if not diff_num(digits[1], dig):
                    digits[3] = sorted(dig)
                elif len(diff_num(digits[4], dig)) == 1:
                    digits[5] = sorted(dig)
                else:
                    digits[2] = sorted(dig)
        res_str= ""
